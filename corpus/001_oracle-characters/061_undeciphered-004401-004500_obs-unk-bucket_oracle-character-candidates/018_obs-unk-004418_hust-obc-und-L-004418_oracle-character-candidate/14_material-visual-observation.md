# Material Visual Observation / 材料图像观察

- Object: `obs-unk-004418`
- Source asset: `001_asset-006011_hust-obc-und-L-004418_glyph.jpg`
- Source route: `HUST-OBC/undeciphered/L/4418/L_？_9892.jpg`
- Observation date: `2026-08-02`
- Review status: direct visual record; identity and reading pending

## Direct Visual Record / 直接可见记录

- A thick rounded horizontal outline occupies the upper area.
- A small oval mark is separated below it.
- A lower U-shaped outline has short upright ends.
- The marks are arranged in three vertical levels.
- 上部区域有一处较厚的圆弧横向轮廓。
- 其下方分离着一处小椭圆痕迹。
- 下部有一处两端带短直立痕迹的 U 形轮廓。
- 各痕迹大致分布在三个上下层次。

## Next Checks / 下一步核查

- Which manifest and checksum verify this local crop?
- Are the three levels separate marks or one damaged outline?
- Which source image records the spacing around the small oval?
- 哪份清单和 checksum 可以核对这张本地裁剪图？
- 三个层次是分离痕迹还是同一轮廓的残缺部分？
- 哪一张来源图像记录小椭圆周围的间距？

This record describes visible marks only. It does not assign a component,
variant, period, inscription identity, reading, meaning, or decipherment.

## Boundary / 边界

- This record preserves source-linked visual material only. It does not
  confirm identity, variant, component, inscription reading, period,
  evolution, or decipherment.
- 中文：本记录只保存有来源的可见材料，不确认字形身份、异体、构件、
  卜辞释读、时期、演变或破译。
