# Source Research Brief / 来源研究资料简报

This brief is the first human reading page for this registered source. It
reports only evidence already recorded in this object folder and names the
limits on research use.

本简报是该已登记来源的首个供人阅读页面。它只陈述本对象目录已经记录的证据，并明确该资料可用于研究的范围和限制。

## Source Identity And Scope / 来源身份与范围
- Title / 标题: 殷墟甲骨文数据库 / Yinxu Oracle Bone Inscriptions Database
- Provider / 提供方: 浙江师范大学; 陈年福教授; 古联（北京）数字传媒科技有限公司
- Evidence level / 证据等级: scholarly_commercial_platform
- Registered scope / 已登记范围: Professional database with inscription browsing,
  transcription search, original text search, oracle dictionary functions and
  many catalog-source abbreviations.
- Source page / 来源页面: https://obid.ancientbooks.cn/

## Actual Registered Evidence / 已登记的实际证据
- Download or access records / 下载或访问记录: 2
- Recorded checksums / 已记录 checksum: 1
- Recorded file sizes / 已记录文件大小: 2
- Package files / 来源包文件: 1
- Field mappings / 字段映射: 2
- Metadata measurements / 元数据测量: 2

- Recorded access item / 已记录访问项: dl-obid-ancientbooks-home; downloaded; 19582
  bytes
- Recorded access item / 已记录访问项: dl-obid-ancientbooks-head-probe-20260727;
  http_error; 0 bytes

## Usable Material Routes / 可用资料路径
- Package material / 来源包资料: dl-obid-ancientbooks-home.html;
  lightweight_html_page; Lightweight source evidence is represented by committed
  provenance, size, checksum, and derived metadata only; ignored tmp downloads
  are not committed as source content.
- Candidate transfer field / 候选转入字段: 卜辞浏览; 释文检索; 原文检索; 甲骨字典 -> source_record ->
  available_access_modes
- Candidate transfer field / 候选转入字段: 合集 H; 中国国家博物馆 GB; 史语所 SG; other
  abbreviations -> external_source_prefix -> external_ref_prefix;
  source_catalog_label

## Research-Use Limits / 研究使用限制
- Rights status / 权利状态: source_marked_risk_noted
- Risk note / 风险提示: Professional/editorial source but commercial platform;
  access and redistribution rights require visible caution.
- Adoption status / 采用状态: adopted_professional_discovery_with_rights_caution
Open 10_source-evidence-dossier.md for full checksums, package rows, field-map
details, and source-specific pending questions.

需要完整 checksum、来源包条目、字段映射和具体待查问题时，请打开 10_source-evidence-dossier.md。

## Boundary / 边界
- not a rights decision
- not corpus import approval
- not a reading or component assignment
- not an inscription identity
- not a decipherment conclusion
- 不是权利结论、语料导入批准、释读、构件归属、卜辞身份或破译结论
