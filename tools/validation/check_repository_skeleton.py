...
def check_hust_obc_undeciphered_candidates(root: Path) -> list[str]:
    ...
    if len(materialized) != 9400:
        issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} should materialize first 9400 rows")
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if index <= 9400:
            if not packet_path:
                issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} first 9400 rows need packet paths")
            elif not (root / packet_path).exists():
                issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} missing packet: {packet_path}")
        elif packet_path:
            issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} rows after first 9400 should not claim packets")
