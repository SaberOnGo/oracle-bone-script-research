# src-evobc Material Access Index / src-evobc 资料访问索引
...
| --- | --- | --- | --- |
| Source summary / 来源摘要 | `README.md` | present | source ID `src-evobc` |
| Human review sheet / 人工复核表 | `06_human-source-review-sheet.md` | present | source provenance and rights checklist |
| Download or access routes / 下载或访问路线 | `02_download-route-index.csv` | route_rows_present | 5 route row(s); statuses: downloaded |
| Package or file manifest routes / 来源包或文件清单路线 | `03_package-route-index.csv` | route_rows_present | 3 route row(s); kinds: large_metadata_json;mapping_json;statistics_xlsx |
| Field maps / 字段映射 | `04_field-map-route-index.csv` | field_rows_present | 2 row(s); target records: graphic_evolution;source_record |
| Downloaded metadata profiles / 已下载 metadata profile | `05_metadata-profile-route-index.csv` | profile_rows_present | 7 row(s); metrics: category_mapping_count;class_count;classes_without_images;era_image_counts;image_reference_count;source_image_counts;worksheet_shape |
...
- Risk note / 风险提示: Useful for evolution-chain experiments, but source texts/websites and image rights need separate review before corpus import.
