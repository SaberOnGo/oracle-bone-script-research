#!/usr/bin/env python3
"""Build a route pack for source access-boundary review.

The pack filters the 104 source-engineering checklist to the
access_boundary_followup lane and pairs each task with the 105 empty result
scaffold. It is routing metadata only: no access outcome is resolved, no
content is imported, and no rights, source-promotion, identity, or decipherment
decision is made.
"""
...
DEFAULT_OUTPUT = Path(
    "corpus/009_statistics-and-derived-features/113_ai-agent-source-access-boundary-review-route-pack.json"
)

UPDATED_AT = "2026-06-19"
ACTION_LANE = "access_boundary_followup"
ROUTE_PACK_ID = "source-access-boundary-review-route-pack-001"
REVIEW_STATUS = "route_pack_pending_access_boundary_review"
RESEARCH_BOUNDARY = "source_access_boundary_review_route_pack_not_access_decision"
CAUTION = (
    "Source access-boundary review route pack only; it does not resolve access "
...
def route_from_rows(checklist_row: dict[str, str], result_row: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "access_outcome": result_row["access_outcome"],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build source access-boundary review route pack.")
