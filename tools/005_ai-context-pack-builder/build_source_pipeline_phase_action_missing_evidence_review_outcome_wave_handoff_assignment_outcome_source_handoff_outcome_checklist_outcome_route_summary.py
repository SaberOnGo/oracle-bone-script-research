#!/usr/bin/env python3
"""Build a route summary for assignment outcome source handoff outcome checklist outcome scaffolds."""
...
AUTOMATION_BOUNDARY = "assignment_outcome_source_handoff_outcome_checklist_outcome_route_summary_only_no_evidence_capture"
RESEARCH_BOUNDARY = (
    "source_pipeline_phase_action_missing_evidence_review_outcome_wave_handoff_assignment_outcome_source_handoff_outcome_checklist_outcome_route_summary_not_scholarship"
)
CAUTION = (
    "This source pipeline missing-evidence assignment outcome source handoff "
    "outcome checklist outcome route summary is routing-only. It is not "
...
def route_files_to_open(row: dict[str, str]) -> str:
    return unique_join(
        [
            SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_OUTCOME_WAVE_HANDOFF_ASSIGNMENT_OUTCOME_SOURCE_HANDOFF_OUTCOME_CHECKLIST_OUTCOME_SCAFFOLD.as_posix(),
            row.get("source_handoff_outcome_checklist_route_summary_path", ""),
            *split_semicolon(row.get("source_handoff_outcome_checklist_outcome_files_to_open", "")),
        ]
    )
...
def route_from_row(index: int, row: dict[str, str]) -> dict[str, object]:
    copied = dict(row)
    copied["source_handoff_outcome_checklist_outcome_route_files_to_open"] = route_files_to_open(row)
    route = {
    ...
    route.update(
        {
            "source_handoff_outcome_checklist_outcome_summary_route_id": (
                "source-pipeline-missing-evidence-review-outcome-wave-handoff-assignment-outcome-source-handoff-outcome-checklist-outcome-summary-route-"
                f"{index:03d}"
            ),
...
def build_route_summary(scaffold_rows: list[dict[str, str]]) -> dict[str, object]:
    ...
    return {
        "route_summary_id": (
            "source-pipeline-missing-evidence-review-outcome-wave-handoff-assignment-outcome-source-handoff-outcome-checklist-outcome-route-summary-001"
        ),
        ...
        "source_handoff_outcome_checklist_outcome_status_counts": status_counts(routes, "source_handoff_outcome_checklist_outcome_status"),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build assignment outcome source handoff outcome checklist outcome route summary.")
    parser.add_argument(
        "--scaffold",
        default=str(
            SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_OUTCOME_WAVE_HANDOFF_ASSIGNMENT_OUTCOME_SOURCE_HANDOFF_OUTCOME_CHECKLIST_OUTCOME_SCAFFOLD
    ...
    print(f"assignment_outcome_source_handoff_outcome_checklist_outcome_summary_routes={data['route_count']} output={args.output}")
    return 0
