#!/usr/bin/env python3
"""Build a route pack for source metadata-profile extraction review.

The pack filters the 104 source-engineering checklist to the
metadata_profile_extraction_planning lane and pairs each task with the 105 empty
result scaffold. It is routing metadata only: no metadata profile is extracted,
no source package is promoted, and no rights, corpus-import, identity, or
decipherment decision is made.
"""
...
DEFAULT_OUTPUT = Path(
    "corpus/009_statistics-and-derived-features/115_ai-agent-source-metadata-profile-review-route-pack.json"
)

UPDATED_AT = "2026-06-19"
ACTION_LANE = "metadata_profile_extraction_planning"
ROUTE_PACK_ID = "source-metadata-profile-review-route-pack-001"
REVIEW_STATUS = "route_pack_pending_metadata_profile_review"
RESEARCH_BOUNDARY = "source_metadata_profile_review_route_pack_not_extraction_decision"
CAUTION = (
    "Source metadata-profile review route pack only; it does not extract "
    "metadata profiles, is not a corpus import, not source promotion, not a "
...
def route_from_rows(checklist_row: dict[str, str], result_row: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "metadata_profile_decision": result_row["metadata_profile_decision"],
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build source metadata-profile review route pack.")
