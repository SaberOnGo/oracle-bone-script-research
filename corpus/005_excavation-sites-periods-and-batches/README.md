# Excavation Sites, Periods, And Batches / 出土地点、时代与批次

English:
This directory will store period, site, pit, batch, collection, and provenance records.
...
- `000_collection-registers/001_institutional-collection-provenance-staging.csv`：从官方图书馆、博物馆和档案数据库页面派生的机构馆藏级出处暂存记录。
- `000_collection-registers/002_ihp-museum-oracle-bone-object-staging.csv`：史语所历史文物陈列馆 Oracle Bones 官方页面列出的 52 个甲骨藏品 item 的对象级暂存记录。

馆藏暂存行保存来源级出处事实，例如馆藏数量、数字化记录数量、具名收藏来源、出土语境和机构 crosswalk 说明。IHP Museum 对象行只把官方 item 页面链接和缩略图 URL 作为 metadata 保存；图片没有下载，item 级权利状态仍需复核。
