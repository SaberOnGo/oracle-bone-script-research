#!/usr/bin/env python3
"""Build a route pack for safe derived-record review.

The pack filters the 104 source-engineering checklist to the
safe_derived_record_decision lane and pairs each task with the 105 empty result
scaffold. It is routing metadata only: no derived record is approved, no source
package is promoted, and no rights, corpus-import, identity, or decipherment
decision is made.
"""
...
DEFAULT_OUTPUT = Path(
    "corpus/009_statistics-and-derived-features/116_ai-agent-source-safe-derived-record-review-route-pack.json"
)

UPDATED_AT = "2026-06-19"
ACTION_LANE = "safe_derived_record_decision"
ROUTE_PACK_ID = "source-safe-derived-record-review-route-pack-001"
REVIEW_STATUS = "route_pack_pending_safe_derived_record_review"
RESEARCH_BOUNDARY = "source_safe_derived_record_review_route_pack_not_promotion_decision"
CAUTION = (
    "Source safe-derived-record review route pack only; it does not decide safe "
    "derived records, is not a corpus import, not source promotion, not a "
    "rights decision, and not an identity or decipherment claim."
)
...
def route_from_rows(checklist_row: dict[str, str], result_row: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "source_coverage_summary_path": SOURCE_COVERAGE_SUMMARY.as_posix(),
        "source_register_path": SOURCE_INDEX.as_posix(),
        "downloaded_metadata_profile_path": DOWNLOADED_METADATA_PROFILE.as_posix(),
        ...
        "safe_derived_record_decision": result_row["safe_derived_record_decision"],
...
def build_route_pack(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "source_paths": {
            ...
            "source_coverage_summary": SOURCE_COVERAGE_SUMMARY.as_posix(),
            "source_register": SOURCE_INDEX.as_posix(),
            "downloaded_metadata_profile": DOWNLOADED_METADATA_PROFILE.as_posix(),
        },
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build source safe-derived-record review route pack.")
