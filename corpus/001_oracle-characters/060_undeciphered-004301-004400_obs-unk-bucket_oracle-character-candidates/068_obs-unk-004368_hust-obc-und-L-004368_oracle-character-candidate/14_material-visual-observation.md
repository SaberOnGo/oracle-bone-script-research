# Material Visual Observation / 材料图像观察

- Object: `obs-unk-004368`
- Source asset: `001_asset-005961_hust-obc-und-L-004368_glyph.jpg`
- Source route: `HUST-OBC/undeciphered/L/4368/L_？_9694.jpg`
- Observation date: `2026-08-02`
- Review status: direct visual record; identity and reading pending

## Direct Visual Record / 直接可见记录

- A broad curved trace rises along the left side.
- A compact rounded mark appears near the upper center.
- A short horizontal or angled trace extends to the right.
- A lower dark trace descends to a narrow uneven terminal.
- 左侧有一条较宽的弧曲痕迹向上延伸。
- 上部中央附近有一处紧密的圆弧状痕迹。
- 一条短横向或斜向痕迹向右外伸。
- 下部有一条深色痕迹向下延伸至狭窄而不齐的末端。

## Next Checks / 下一步核查

- Which source manifest and checksum verify this local crop?
- Is the central rounded mark separate from the left curve?
- Can a wider plate view clarify the lower narrow terminal?
- 哪条来源清单和校验和可以核对本地裁剪图？
- 中央圆弧痕是否与左侧弧线分开？
- 是否有更宽图版可核对下部狭窄末端？

This record describes visible marks only. It does not assign a component,
variant, period, inscription identity, reading, meaning, or decipherment.

## Boundary / 边界

- This record preserves source-linked visual material only. It does not
  confirm identity, variant, component, inscription reading, period,
  evolution, or decipherment.
- 中文：本记录只保存有来源的可见材料，不确认字形身份、异体、构件、
  卜辞释读、时期、演变或破译。
