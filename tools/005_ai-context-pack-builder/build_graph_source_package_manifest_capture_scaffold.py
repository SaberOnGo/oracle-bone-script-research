#!/usr/bin/env python3
"""Build a package-manifest evidence capture scaffold from the third handoff wave."""
...
FIELDNAMES = [
    ...
    "source_package_file_manifest_path",
    "source_package_file_manifest_row_status",
...
def _capture_row(index: int, item: dict[str, object]) -> dict[str, str]:
    return {
        "capture_row_id": f"graph-source-evidence-package-manifest-capture-{index:03d}",
        ...
        "source_package_file_manifest_path": SOURCE_PACKAGE_FILE_MANIFEST_PATH,
        ...
        "caution": (
            "Empty capture scaffold for package-manifest evidence only. Fill "
            "this row only after opening the source package file manifest and "
            "cited route files; do not treat file size, checksum, storage "
            "boundary, rights status, or handling strategy fields as reviewed "
            "until a source-marked review records them elsewhere. Do not use "
            "this row as collected evidence, source promotion, or a "
...
def build_capture_scaffold(handoff_scaffold: dict[str, object]) -> list[dict[str, str]]:
    ...
    if any(row["target_evidence_section"] != "package_manifest" for row in rows):
        raise ValueError("capture scaffold only supports package_manifest rows")
    return rows
