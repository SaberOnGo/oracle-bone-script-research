#!/usr/bin/env python3
"""Build a route summary for assignment outcome source handoff outcome scaffolds."""
...
AUTOMATION_BOUNDARY = "assignment_outcome_source_handoff_outcome_route_summary_only_no_evidence_capture"
RESEARCH_BOUNDARY = (
...
CAUTION = (
    ...
    "outcome route summary is routing-only. It is not collected evidence, not "
    "a reviewed outcome, not a rights decision, not source promotion, not a "
...
def route_files_to_open(row: dict[str, str]) -> str:
    return unique_join(
        [
            SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_OUTCOME_WAVE_HANDOFF_ASSIGNMENT_OUTCOME_SOURCE_HANDOFF_OUTCOME_SCAFFOLD.as_posix(),
            row.get("source_handoff_route_summary_path", ""),
            *split_semicolon(row.get("source_handoff_route_files_to_open", "")),
        ]
    )
...
def route_from_row(index: int, row: dict[str, str]) -> dict[str, object]:
    copied = dict(row)
    copied["source_handoff_outcome_route_files_to_open"] = route_files_to_open(row)
    route = {
    ...
    route.update(
        {
            "source_handoff_outcome_summary_route_id": (
                "source-pipeline-missing-evidence-review-outcome-wave-handoff-assignment-outcome-source-handoff-outcome-summary-route-"
                f"{index:03d}"
            ),
...
def build_route_summary(scaffold_rows: list[dict[str, str]]) -> dict[str, object]:
    ...
    return {
        ...
        "source_handoff_outcome_status_counts": status_counts(routes, "source_handoff_outcome_status"),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build assignment outcome source handoff outcome route summary.")
    parser.add_argument(
        "--scaffold",
        default=str(
            SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_OUTCOME_WAVE_HANDOFF_ASSIGNMENT_OUTCOME_SOURCE_HANDOFF_OUTCOME_SCAFFOLD
    ...
    print(f"assignment_outcome_source_handoff_outcome_summary_routes={data['route_count']} output={args.output}")
    return 0
