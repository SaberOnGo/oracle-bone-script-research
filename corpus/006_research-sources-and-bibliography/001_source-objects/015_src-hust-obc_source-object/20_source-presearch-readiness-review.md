...
objects with local image derivatives. `5535` objects have a separate
object-local `14_material-visual-observation.md` record, while `5461` still
need a direct visual observation.

截至 2026-08-01，仓库审计显示 HUST-OBC 的 `10996` 个单字对象都有本地
图像派生件。其中 `5535` 个对象已有对象内独立的
`14_material-visual-observation.md`，仍有 `5461` 个对象需要直接图像观察。
