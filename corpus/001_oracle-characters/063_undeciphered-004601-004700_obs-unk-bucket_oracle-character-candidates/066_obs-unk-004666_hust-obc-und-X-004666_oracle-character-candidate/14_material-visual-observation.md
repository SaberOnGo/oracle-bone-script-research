# Material Visual Observation / 材料图像观察

- Object: `obs-unk-004666`
- Source asset: `001_asset-006259_hust-obc-und-X-004666_glyph.jpg`
- Source route: `HUST-OBC/undeciphered/X/222/X_？_8393_1.png`
- Observation date: `2026-08-02`
- Review status: direct visual record; image quality review pending

## Direct Visual Record / 直接可见记录

- The derivative has a dark background with broken light regions.
- A light diagonal region appears in the upper part of the image.
- A second light cluster appears lower and slightly right of center.
- Speckled edges make the source stroke boundaries unstable.
- No reliable internal contour can be assigned from this derivative.
- 当前派生图有深色背景，并带有断续的浅色区域。
- 图像上部出现一条浅色斜向区域。
- 下部偏右位置另有一组浅色聚集。
- 斑点状边缘使来源笔画边界不稳定。
- 不能依据该派生图确定可靠的内部轮廓。
- These observations describe image quality and visible contrast only; no
  component, variant, or reading is assigned.

## Follow-up Questions / 待查问题

- Can an original grayscale source replace this dark-background derivative?
- Which preprocessing step fragmented the light regions?
- Which plate or catalog record identifies the source image?
- 是否可以用原始灰度来源替换当前深色背景派生图？
- 哪一步预处理切碎了浅色区域？
- 哪条图版或著录记录能够确定来源图像？

This record describes visible contrast and image quality only. It does not
assign a component, variant, period, inscription identity, reading, meaning,
or decipherment.
