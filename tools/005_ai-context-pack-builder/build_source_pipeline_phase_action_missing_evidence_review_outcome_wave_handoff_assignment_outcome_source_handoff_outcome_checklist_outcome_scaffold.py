#!/usr/bin/env python3
"""Build empty human-gated outcome scaffold rows from the 177 checklist route summary."""
...
SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_OUTCOME_WAVE_HANDOFF_ASSIGNMENT_OUTCOME_SOURCE_HANDOFF_OUTCOME_CHECKLIST_ROUTE_SUMMARY = (
    STAT_DIR
    / "177_source-pipeline-phase-action-missing-evidence-review-outcome-wave-handoff-assignment-outcome-source-handoff-outcome-checklist-route-summary.json"
)
DEFAULT_OUTPUT = (
    STAT_DIR
    / "178_source-pipeline-phase-action-missing-evidence-review-outcome-wave-handoff-assignment-outcome-source-handoff-outcome-checklist-outcome-scaffold.csv"
)

UPDATED_AT = "2026-06-19"
SOURCE_HANDOFF_OUTCOME_CHECKLIST_OUTCOME_STATUS = "not_started"
...
AUTOMATION_BOUNDARY = "assignment_outcome_source_handoff_outcome_checklist_outcome_scaffold_only_no_evidence_capture"
RESEARCH_BOUNDARY = (
    "source_pipeline_phase_action_missing_evidence_review_outcome_wave_handoff_assignment_outcome_source_handoff_outcome_checklist_outcome_scaffold_not_scholarship"
)
...
CAUTION = (
    ...
    "outcome checklist file is a human-gated outcome scaffold. It is not "
...
def files_to_open(route: dict[str, object]) -> str:
    return unique_join(
        [
            ...
            SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_OUTCOME_WAVE_HANDOFF_ASSIGNMENT_OUTCOME_SOURCE_HANDOFF_OUTCOME_CHECKLIST_ROUTE_SUMMARY.as_posix(),
            *join_semicolon(route.get("source_handoff_outcome_checklist_route_files_to_open")).split(";"),
        ]
    )
...
def build_outcome_scaffold_rows(route_summary: dict[str, object]) -> list[dict[str, str]]:
    ...
    for index, route in enumerate(routes, start=1):
        ...
        row = {
            "source_handoff_outcome_checklist_outcome_scaffold_id": (
                "source-pipeline-missing-evidence-review-outcome-wave-handoff-assignment-outcome-source-handoff-outcome-checklist-outcome-scaffold-"
                f"{index:03d}"
            )
        }
        ...
        row.update(
            {
                ...
                "source_handoff_outcome_checklist_outcome_scaffold_path": DEFAULT_OUTPUT.as_posix(),
                "source_handoff_outcome_checklist_outcome_files_to_open": files_to_open(route_row),
                "source_handoff_outcome_checklist_outcome_status": SOURCE_HANDOFF_OUTCOME_CHECKLIST_OUTCOME_STATUS,
                "source_metadata_outcome_reviewed": "",
                "access_boundary_outcome_reviewed": "",
                "download_or_access_outcome_reviewed": "",
                "large_source_register_outcome_reviewed": "",
                ...
                "reviewed_evidence_paths": "",
                "reviewed_outcome_summary": "",
                "remaining_blockers_reviewed": join_semicolon(route_row.get("blocking_condition")),
                "required_followup_reviewed": "",
                "human_reviewer_id": "",
                "human_review_date": "",
                "human_review_notes": "",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build assignment outcome source handoff outcome checklist outcome scaffold.")
    parser.add_argument(
        "--route-summary",
        default=str(
            SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_OUTCOME_WAVE_HANDOFF_ASSIGNMENT_OUTCOME_SOURCE_HANDOFF_OUTCOME_CHECKLIST_ROUTE_SUMMARY
    ...
    print(f"assignment_outcome_source_handoff_outcome_checklist_outcome_scaffold_rows={len(rows)} output={args.output}")
    return 0
