# Material Visual Observation / 材料图像观察

- Object: `obs-unk-004431`
- Source asset: `001_asset-006024_hust-obc-und-L-004431_glyph.jpg`
- Source route: `HUST-OBC/undeciphered/L/4431/L_？_9930.jpg`
- Observation date: `2026-08-02`
- Review status: direct visual record; identity and reading pending

## Direct Visual Record / 直接可见记录

- A long central stem crosses a horizontal middle trace.
- An upper left diagonal trace meets the central stem.
- A curved trace extends toward the right side.
- Lower marks are darker and curve unevenly around the stem.
- 一条较长的中央竖痕与中部横向痕迹相交。
- 左上方斜向痕迹与中央竖痕相遇。
- 一条弯曲痕迹向右侧延伸。
- 下部痕迹颜色较深，并围绕中央竖痕不均匀弯曲。

## Next Checks / 下一步核查

- Which manifest and checksum verify this local crop?
- Does the right curved trace continue outside the crop?
- Which wider image confirms the darker lower curves?
- 哪份清单和 checksum 可以核对这张本地裁剪图？
- 右侧弯曲痕迹是否在裁剪范围外继续？
- 哪一张更宽图像可以确认下部较深的弧线？

This record describes visible marks only. It does not assign a component,
variant, period, inscription identity, reading, meaning, or decipherment.

## Boundary / 边界

- This record preserves source-linked visual material only. It does not
  confirm identity, variant, component, inscription reading, period,
  evolution, or decipherment.
- 中文：本记录只保存有来源的可见材料，不确认字形身份、异体、构件、
  卜辞释读、时期、演变或破译。
