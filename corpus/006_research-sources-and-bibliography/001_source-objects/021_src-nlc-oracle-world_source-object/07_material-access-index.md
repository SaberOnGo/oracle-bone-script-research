# src-nlc-oracle-world Material Access Index / src-nlc-oracle-world 资料访问索引
...
| --- | --- | --- | --- |
| Source summary / 来源摘要 | `README.md` | present | source ID `src-nlc-oracle-world` |
...
| Package or file manifest routes / 来源包或文件清单路线 | `03_package-route-index.csv` | route_rows_present | 2 route row(s); kinds: lightweight_pdf |
| Field maps / 字段映射 | `04_field-map-route-index.csv` | field_rows_present | 4 row(s); target records: oracle_inscription;oracle_inscription; excavation_or_collection_context;source_reference |
| Downloaded metadata profiles / 已下载 metadata profile | `05_metadata-profile-route-index.csv` | profile_rows_present | 7 row(s); metrics: nlc_heji_cataloged_count;nlc_oracle_bone_holding_count;nlc_yinqi_cuibian_cataloged_count;oracle_world_object_image_count;oracle_world_object_record_count;oracle_world_rubbing_image_count;oracle_world_rubbing_record_count |
...
- Risk note / 风险提示: Official NLC materials confirm collection/database scope but not a stable current bulk endpoint; do not use mirrored summaries or third-party portals as source data.
