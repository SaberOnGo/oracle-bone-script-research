# src-metmuseum-oracle-bone Material Access Index / src-metmuseum-oracle-bone 资料访问索引
...
| --- | --- | --- | --- |
| Source summary / 来源摘要 | `README.md` | present | source ID `src-metmuseum-oracle-bone` |
...
| Package or file manifest routes / 来源包或文件清单路线 | `03_package-route-index.csv` | route_rows_present | 2 route row(s); kinds: lightweight_api_json |
| Field maps / 字段映射 | `04_field-map-route-index.csv` | field_rows_present | 3 row(s); target records: asset_metadata;asset_metadata; excavation_or_collection_context |
| Downloaded metadata profiles / 已下载 metadata profile | `05_metadata-profile-route-index.csv` | profile_rows_present | 6 row(s); metrics: accession_number;is_public_domain;object_id |
...
- Risk note / 风险提示: The Met API marks selected objects as public domain, but object metadata and image URLs can be revised and should retain object-level timestamps and source URLs.
