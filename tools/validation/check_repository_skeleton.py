...
def check_hust_obc_undeciphered_candidates(root: Path) -> list[str]:
    ...
    if len(materialized) != 2300:
        issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} should materialize first 2300 rows")
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if index <= 2300:
            if not packet_path:
                issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} first 2300 rows need packet paths")
            elif not (root / packet_path).exists():
                issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} missing packet: {packet_path}")
        elif packet_path:
            issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} rows after first 2300 should not claim packets")
