#!/usr/bin/env python3
"""Build a precheck checklist for missing-evidence review handoffs.

The checklist gives later reviewers a per-handoff precheck surface before any
missing-evidence source outcome is recorded. It does not collect evidence,
assign owners, decide rights, promote sources, import corpus rows, or make
...
DEFAULT_OUTPUT = STAT_DIR / "153_source-pipeline-phase-action-missing-evidence-review-handoff-checklist.csv"

UPDATED_AT = "2026-06-19"
CHECKLIST_STATUS = "not_started"
AUTOMATION_BOUNDARY = "handoff_precheck_only_no_missing_evidence_outcome_capture"
RESEARCH_BOUNDARY = "source_pipeline_phase_action_missing_evidence_review_handoff_checklist_not_scholarship"
CAUTION = (
    "This source pipeline missing-evidence handoff review checklist is a precheck "
...
REQUIRED_PRECHECK_STEPS = [
    "verify_handoff_row_against_152",
    "open_151_route_pack",
    "open_149_result_scaffold",
    "open_148_review_draft_manifest",
    "open_all_files_to_open_before_review",
...
def handoff_files_to_open(handoff: dict[str, object]) -> str:
    paths = [
        SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_HANDOFF_SCAFFOLD.as_posix(),
...
def row_from_handoff(index: int, handoff: dict[str, object]) -> dict[str, str]:
    return {
        "handoff_review_checklist_id": f"source-pipeline-missing-evidence-review-handoff-checklist-{index:03d}",
        ...
        "handoff_scaffold_path": SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_HANDOFF_SCAFFOLD.as_posix(),
        ...
        "handoff_files_to_open": handoff_files_to_open(handoff),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build source pipeline missing-evidence handoff checklist.")
    parser.add_argument("--handoff-scaffold", default=str(SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_HANDOFF_SCAFFOLD))
    ...
    print(f"missing_evidence_review_handoff_checklist_rows={len(rows)} output={args.output}")
    return 0
