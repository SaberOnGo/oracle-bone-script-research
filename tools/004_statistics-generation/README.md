...
- `build_core_corpus_phase_gap_review_handoff_route_summary.py` generates `203_core-corpus-phase-gap-review-handoff-route-summary.json` from the `202` handoff checklist. It indexes the 20 planned handoff checklist rows as route-summary entries for navigation and status aggregation only, without collecting evidence, recording reviewed outcomes, deciding rights, promoting sources or candidates, importing formal corpus records, or making decipherment claims.
- `build_core_corpus_phase_gap_review_outcome_scaffold.py` generates `204_core-corpus-phase-gap-review-outcome-scaffold.csv` from the `203` handoff route summary. It creates the human-fillable outcome surface for later core-corpus phase gap review while keeping evidence, reviewed outcomes, rights decisions, source or candidate promotion, corpus import, identity/component/evolution claims, and decipherment claims unset.
- `build_core_corpus_phase_gap_review_outcome_route_pack.py` generates `205_core-corpus-phase-gap-review-outcome-route-pack.json` from the `204` outcome scaffold. It indexes the 20 empty outcome scaffold rows for routing before human review, without collecting evidence, recording reviewed outcomes, deciding rights, promoting sources or candidates, importing corpus records, or making identity, component, evolution, or decipherment claims.
- `build_core_corpus_phase_gap_review_outcome_handoff_scaffold.py` generates `206_core-corpus-phase-gap-review-outcome-handoff-scaffold.json` from the `205` outcome route pack. It wraps the 20 outcome routes as planned-only handoffs for later human review without assigning owners, collecting evidence, recording outcomes, deciding rights, promoting sources or candidates, importing corpus records, or making identity, component, evolution, or decipherment claims.
- `build_core_corpus_phase_gap_review_outcome_handoff_checklist.py` generates `207_core-corpus-phase-gap-review-outcome-handoff-checklist.csv` from the `206` outcome handoff scaffold. It gives each planned outcome handoff one precheck row for opening the handoff scaffold, outcome route pack, outcome scaffold, routed specialized checklist, and linked route files before any human-gated outcome review is recorded.
- `build_core_corpus_phase_gap_review_outcome_handoff_route_summary.py` generates `208_core-corpus-phase-gap-review-outcome-handoff-route-summary.json` from the `207` outcome handoff checklist. It indexes the 20 precheck rows as routing and status-aggregation entries only, without collecting evidence, recording reviewed outcomes, deciding rights, promoting sources or candidates, importing corpus records, or making identity, component, evolution, or decipherment claims.
- `build_core_corpus_phase_gap_review_outcome_assignment_plan.py` generates `209_core-corpus-phase-gap-review-outcome-assignment-plan.json` from the `208` outcome handoff route summary. It groups the 20 planned outcome routes into phase-status review waves and corpus-area workstreams for navigation only, without assigning reviewers, collecting evidence, recording reviewed outcomes, deciding rights, promoting sources or candidates, importing corpus records, or making identity, component, evolution, or decipherment claims.
- `build_core_corpus_phase_gap_review_outcome_assignment_checklist.py` generates `210_core-corpus-phase-gap-review-outcome-assignment-checklist.csv` from the `209` outcome assignment plan. It expands each planned assignment into a CSV precheck row that opens the plan, route summary, and routed files while keeping evidence collection, reviewed outcomes, rights decisions, source or candidate promotion, corpus import, identity, component, evolution, and decipherment claims unset.
- `build_core_corpus_phase_gap_review_outcome_assignment_outcome_scaffold.py` generates `211_core-corpus-phase-gap-review-outcome-assignment-outcome-scaffold.csv` from the `210` assignment checklist. It creates empty human-fillable outcome rows for the planned assignments while preserving no evidence collection, no reviewed outcome, no rights decision, no source or candidate promotion, no corpus import, and no identity, component, evolution, or decipherment claim.
- `build_core_corpus_phase_gap_review_outcome_assignment_outcome_route_summary.py` generates `212_core-corpus-phase-gap-review-outcome-assignment-outcome-route-summary.json` from the `211` assignment outcome scaffold. It indexes the empty outcome rows by assignment wave, corpus area, phase status, and status counters for navigation only, without collecting evidence, recording reviewed outcomes, deciding rights, promoting sources or candidates, importing corpus records, or making identity, component, evolution, or decipherment claims.
...
- `build_core_corpus_phase_gap_review_handoff_route_summary.py` 基于 `202` handoff checklist 生成 `203_core-corpus-phase-gap-review-handoff-route-summary.json`。它把 20 条计划中的 handoff checklist 行索引为 route-summary 条目，只用于导航和状态汇总，不采集证据、不记录已复核 outcome、不裁定权利、不提升来源或候选、不导入正式语料，也不提出释读结论。
- `build_core_corpus_phase_gap_review_outcome_scaffold.py` 基于 `203` handoff route summary 生成 `204_core-corpus-phase-gap-review-outcome-scaffold.csv`。它为后续核心语料阶段缺口复核建立人工可填写 outcome 表面，同时保持证据、已复核 outcome、权利决定、来源或候选提升、语料导入、身份/构件/演化判断和释读结论均未设置。
- `build_core_corpus_phase_gap_review_outcome_route_pack.py` 基于 `204` outcome scaffold 生成 `205_core-corpus-phase-gap-review-outcome-route-pack.json`。它只为 20 条空 outcome scaffold 行建立人工复核前的路由索引，不采集证据、不记录已复核 outcome、不裁定权利、不提升来源或候选、不导入语料，也不做身份、构件、演化或释读结论。
- `build_core_corpus_phase_gap_review_outcome_handoff_scaffold.py` 基于 `205` outcome route pack 生成 `206_core-corpus-phase-gap-review-outcome-handoff-scaffold.json`。它把 20 条 outcome route 包装为后续人工复核使用的 planned-only handoff，不分配 owner、不采集证据、不记录 outcome、不裁定权利、不提升来源或候选、不导入语料，也不做身份、构件、演化或释读结论。
- `build_core_corpus_phase_gap_review_outcome_handoff_checklist.py` 基于 `206` outcome handoff scaffold 生成 `207_core-corpus-phase-gap-review-outcome-handoff-checklist.csv`。它为每条 planned outcome handoff 提供一行前置检查入口，用于在任何人工门控 outcome review 记录前打开 handoff scaffold、outcome route pack、outcome scaffold、已路由专项清单和相关 route files。
- `build_core_corpus_phase_gap_review_outcome_handoff_route_summary.py` 基于 `207` outcome handoff checklist 生成 `208_core-corpus-phase-gap-review-outcome-handoff-route-summary.json`。它仅把 20 条前置检查行索引为路由和状态聚合条目，不采集证据、不记录已复核 outcome、不裁定权利、不提升来源或候选、不导入语料，也不做身份、构件、演化或释读结论。
English supplement:
- `build_core_corpus_phase_gap_review_outcome_assignment_plan.py` generates `209_core-corpus-phase-gap-review-outcome-assignment-plan.json` from the `208` outcome handoff route summary. It groups the 20 planned outcome routes by phase status and corpus area for navigation only; it does not assign reviewers, collect evidence, record reviewed outcomes, decide rights, promote sources or candidates, import corpus records, or make identity, component, evolution, or decipherment claims.

Simplified Chinese supplement:
- `build_core_corpus_phase_gap_review_outcome_assignment_plan.py` 基于 `208` outcome handoff route summary 生成 `209_core-corpus-phase-gap-review-outcome-assignment-plan.json`。它仅把 20 条计划中的 outcome 路线按 phase status 和 corpus area 分组用于导航；不分配复核者、不采集证据、不记录已复核 outcome、不裁定权利、不提升来源或候选、不导入语料，也不做身份、构件、演化或释读结论。
- `build_core_corpus_phase_gap_review_outcome_assignment_checklist.py` 基于 `209` outcome assignment plan 生成 `210_core-corpus-phase-gap-review-outcome-assignment-checklist.csv`。它把每个计划项展开为 CSV 前置检查行，用于打开 plan、route summary 和已路由文件；证据采集、已复核 outcome、权利裁定、来源或候选提升、语料导入、身份、构件、演化和释读结论仍保持未设置。
- `build_core_corpus_phase_gap_review_outcome_assignment_outcome_scaffold.py` 基于 `210` assignment checklist 生成 `211_core-corpus-phase-gap-review-outcome-assignment-outcome-scaffold.csv`。它为计划中的 assignment 建立空白、可人工填写的 outcome 行，同时保持不采集证据、不记录已复核 outcome、不裁定权利、不提升来源或候选、不导入语料，也不做身份、构件、演化或释读结论。
- `build_core_corpus_phase_gap_review_outcome_assignment_outcome_route_summary.py` 基于 `211` assignment outcome scaffold 生成 `212_core-corpus-phase-gap-review-outcome-assignment-outcome-route-summary.json`。它按 assignment wave、corpus area、phase status 和状态计数索引空白 outcome 行，仅用于导航；不采集证据、不记录已复核 outcome、不裁定权利、不提升来源或候选、不导入语料，也不做身份、构件、演化或释读结论。
