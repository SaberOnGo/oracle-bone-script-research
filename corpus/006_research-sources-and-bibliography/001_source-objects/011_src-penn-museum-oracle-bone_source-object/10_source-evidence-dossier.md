...
- Source URL / 来源链接: https://www.penn.museum/collections/object/76960
...
### Route 001
- Download ID / 下载 ID: dl-penn-museum-49-14-7a
- Artifact kind / 资料类型: museum_object_page
- Status / 状态: downloaded
- HTTP status / HTTP 状态: 200
- File size bytes / 文件大小 bytes: 29678
- Checksum SHA-256 / checksum SHA-256:
  331982b694181de3e752d3e650c0de903afed14bd13acbf34b5c689a889ffa12
- Commit policy / 提交策略: download_to_tmp_log_checksum_only
- Local temp path / 本地临时路径: tmp/source_downloads/dl-penn-museum-49-14-7a.html
...
### Route 001
- Package file ID / 来源包文件 ID: pkg-file-000027
- Source package ID / 来源包 ID: light-src-penn-museum-oracle-bone
- File name / 文件名: dl-penn-museum-49-14-7a.html
...
### Route 001
- Field map ID / 字段映射 ID: field-map-000046
- Source level / 来源层级: object_record_field
- Source field or unit / 来源字段或单位: Object Number
- Source meaning / 来源含义: Penn Museum object number such as 49-14-7A
...
- Import action / 导入动作: Use as object-level external reference for university
  museum collection staging
- Rights boundary / 权利边界: Object number is museum-local and must not replace
  project IDs
- Evidence download ID / 证据下载 ID: dl-penn-museum-49-14-7a
- Review status / 复核状态: reviewed_metadata_only

### Route 002
- Field map ID / 字段映射 ID: field-map-000047
- Source level / 来源层级: object_record_field
- Source field or unit / 来源字段或单位: Provenience; Period; Date Made
- Source meaning / 来源含义: Penn Museum provenience and dating fields for Anyang
  and Shang Dynasty
- Target record type / 目标记录类型: excavation_or_collection_context
- Target project field / 目标字段: provenience; historical_period; date_made
- Import action / 导入动作: Use as collection-context metadata for object staging
- Rights boundary / 权利边界: Display/catalog metadata must be cross-checked before
  inscription-level historical claims
- Evidence download ID / 证据下载 ID: dl-penn-museum-49-14-7a
- Review status / 复核状态: reviewed_metadata_only

### Route 003
- Field map ID / 字段映射 ID: field-map-000048
- Source level / 来源层级: object_record_field
- Source field or unit / 来源字段或单位: Materials; Height; Width; Credit Line
- Source meaning / 来源含义: Penn Museum material dimensions and acquisition credit
  line
- Target record type / 目标记录类型: asset_metadata
- Target project field / 目标字段: materials; dimensions; credit_line
- Import action / 导入动作: Use as asset/object metadata for future image and object
  provenance workflow
- Rights boundary / 权利边界: Raw images and reuse rights require separate
  object-level review
- Evidence download ID / 证据下载 ID: dl-penn-museum-49-14-7a
- Review status / 复核状态: reviewed_metadata_only
...
### Route 001
- Profile ID / 概况 ID: metadata-profile-000034
- Evidence download ID / 证据下载 ID: dl-penn-museum-49-14-7a
- Metadata file / 元数据文件: 49-14-7A
- Profile metric / 概况指标: object_number
- Profile value / 概况值: 49-14-7A
- Profile unit / 概况单位: museum_object_number
- Import relevance / 导入相关性: Confirms official Penn Museum object number for
  oracle bone staging
- Caution / 提醒: Object number is not a project character or inscription ID
- Review status / 复核状态: reviewed_metadata_only

### Route 002
- Profile ID / 概况 ID: metadata-profile-000035
- Evidence download ID / 证据下载 ID: dl-penn-museum-49-14-7a
- Metadata file / 元数据文件: 49-14-7A
- Profile metric / 概况指标: provenience
- Profile value / 概况值: Anyang
- Profile unit / 概况单位: place_name
- Import relevance / 导入相关性: Records official Penn Museum provenience field for
  the object
- Caution / 提醒: Provenience field needs cross-source context before
  excavation-level claims
- Review status / 复核状态: reviewed_metadata_only

### Route 003
- Profile ID / 概况 ID: metadata-profile-000036
- Evidence download ID / 证据下载 ID: dl-penn-museum-49-14-7a
- Metadata file / 元数据文件: 49-14-7A
- Profile metric / 概况指标: period
- Profile value / 概况值: Shang Dynasty
- Profile unit / 概况单位: period_label
- Import relevance / 导入相关性: Records official Penn Museum period label for
  object-level context
- Caution / 提醒: Period label is museum metadata and not an inscription-specific
  dating proof
- Review status / 复核状态: reviewed_metadata_only

### Route 004
- Profile ID / 概况 ID: metadata-profile-000037
- Evidence download ID / 证据下载 ID: dl-penn-museum-49-14-7a
- Metadata file / 元数据文件: 49-14-7A
- Profile metric / 概况指标: current_location
- Profile value / 概况值: Asia Galleries - On Display
- Profile unit / 概况单位: display_location
- Import relevance / 导入相关性: Records official Penn Museum display-location
  metadata
- Caution / 提醒: Display location can change and should retain download timestamp
- Review status / 复核状态: reviewed_metadata_only
