# src-sinica-da-xiaoxuetang-site Material Access Index / src-sinica-da-xiaoxuetang-site 资料访问索引
...
| --- | --- | --- | --- |
| Source summary / 来源摘要 | `README.md` | present | source ID `src-sinica-da-xiaoxuetang-site` |
| Human review sheet / 人工复核表 | `06_human-source-review-sheet.md` | present | source provenance and rights checklist |
| Download or access routes / 下载或访问路线 | `02_download-route-index.csv` | route_rows_present | 1 route row(s); statuses: download_error |
| Package or file manifest routes / 来源包或文件清单路线 | `03_package-route-index.csv` | not_present_in_current_registers | 0 route row(s); kinds: none |
| Field maps / 字段映射 | `04_field-map-route-index.csv` | field_rows_present | 1 row(s); target records: source_reference |
| Downloaded metadata profiles / 已下载 metadata profile | `05_metadata-profile-route-index.csv` | profile_rows_present | 3 row(s); metrics: xiaoxuetang_dictionary_index_total_over;xiaoxuetang_glyph_total_over;xiaoxuetang_phonology_total_over |
...
- Risk note / 风险提示: Portal description is database-level evidence and does not provide row-level character or inscription records.
