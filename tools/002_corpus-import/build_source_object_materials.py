...
def source_literature_scope_review_text(source: dict[str, str]) -> str:
    ...
    elif source["source_id"] == "src-gbedobc":
        lines.extend(
            [
                "## Primary Publication / 主要论文",
                *bullet(
                    "Citation / 引用",
                    "Jiao, Q., Wu, J., Liu, Q. et al. A graph-based evolutionary "
                    "dataset for oracle bone characters from inscriptions to "
                    "modern Chinese scripts. npj Heritage Science 13, 369 "
                    "(2025).",
                ),
                *bullet(
                    "DOI and dates / DOI 与日期",
                    "DOI: https://doi.org/10.1038/s40494-025-01951-0. Received "
                    "2025-04-21, accepted 2025-07-13, and published 2025-07-26.",
                ),
                *bullet(
                    "Responsible roles / 责任分工",
                    "The paper records Qingju Jiao, Jingwen Wu, Qi Liu, Han Zhang, "
                    "Zhan Zhang, Bang Li, Jing Xiong, Guoying Liu, and Yongge Liu. "
                    "It assigns design, collection, processing, validation, and "
                    "writing roles rather than a single decipherment authority.",
                ),
                *bullet(
                    "Evidence level / 证据等级",
                    "Primary peer-reviewed dataset article; it reports a graph "
                    "representation and computational comparison, not an "
                    "archaeological catalog identity or accepted reading.",
                ),
                "",
                "## Reported Dataset And Source Scope / 论文报告的数据范围",
                *bullet(
                    "Reported scale / 论文报告规模",
                    "The paper reports 756 groups and 3,780 Chinese characters "
                    "across five stages: oracle bone, bronze, seal, official, and "
                    "regular script. Local routes record this as source scope, not "
                    "as 3,780 imported corpus records.",
                ),
                *bullet(
                    "Upstream image source / 上游图像来源",
                    "The paper says its 4,860 source images in 972 groups came from "
                    "an earlier image-based dataset, with Guo Xue Da Shi and the "
                    "Chinese Character Etymology and Evolution Dictionary named as "
                    "primary source routes.",
                ),
                *bullet(
                    "Stage and image boundary / 阶段与图像边界",
                    "The five-stage graph groups are not the six-stage EVOBC scope. "
                    "Do not merge their stages, labels, images, or group IDs without "
                    "a source-level crosswalk.",
                ),
                "",
                "## Reported Processing And Graph Method / 处理与图结构方法",
                *bullet(
                    "Image preparation / 图像预处理",
                    "The paper reports grayscale conversion and normalization to "
                    "105 by 105 pixels before comparing image and graph routes.",
                ),
                *bullet(
                    "Graph construction / 图结构构建",
                    "The reported method extracts key points, connects them by "
                    "stroke-based relations, compresses the graph, and compares "
                    "adjacency-matrix features across stages.",
                ),
                *bullet(
                    "Validation / 技术验证",
                    "DHC-E, FEATHER, and GL2vec are compared with image-based "
                    "methods for computational similarity and stability. These "
                    "scores validate a method, not a historical correspondence.",
                ),
                "",
                "## Citation And Access Relations / 引用与访问关系",
                *bullet(
                    "Data and code route / 数据与代码路线",
                    "The paper provides 3,780 images and graph matrices, plus "
                    "derived vectors, through the BrisksHan/GBEDOBC GitHub route. "
                    "The local package keeps provenance and checksum records only.",
                ),
                *bullet(
                    "Upstream citations / 上游引用路线",
                    "The article cites the earlier image-based evolution dataset, "
                    "EVOBC, Guo Xue Da Shi, and the Chinese Character Etymology and "
                    "Evolution Dictionary. Each needs independent bibliography and "
                    "rights review.",
                ),
                *bullet(
                    "Human dossier transfer / 人类档案转入",
                    "A graph edge may guide comparison, but a human dossier must "
                    "retain the source image, page or catalog trail, modern label, "
                    "graph route, and pending review status separately.",
                ),
                "",
                "## Limits, Disputes, And Rights / 限制、争议与权利",
                *bullet(
                    "Graph identity boundary / 图节点身份边界",
                    "A node or edge in a graph is a dataset representation. It does "
                    "not by itself establish a glyph identity, component, variant, "
                    "or evolution relationship.",
                ),
                *bullet(
                    "Source and label disputes / 来源与标签争议",
                    "The dataset inherits upstream image, label, stage, and source "
                    "choices. The paper does not resolve every catalog, modern-label, "
                    "or historical correspondence question.",
                ),
                *bullet(
                    "License / 许可",
                    "The article states CC BY-NC-ND 4.0 and warns that third-party "
                    "material may have separate credit lines. Keep repository and "
                    "upstream book or website rights as separate review items.",
                ),
                "",
                "## Concrete Bibliography Checks / 具体文献核查",
    ...
    elif source["source_id"] == "src-open-oracle":
        lines.extend(
            [
                "## Repository Research Scope / 仓库研究范围",
                *bullet(
                    "Repository function / 仓库功能",
                    "The Open-Oracle README describes a project hub that curates "
                    "open datasets, benchmarks, codebases, papers, and online "
                    "resources for AI-assisted oracle-bone research.",
                ),
                *bullet(
                    "Provider and maintenance / 提供方与维护",
                    "The repository is maintained by the Yuliang Liu research group. "
                    "The current public page states last maintained 2026-07-02; the "
                    "local README checksum remains the historical snapshot to cite.",
                ),
                *bullet(
                    "Evidence level / 证据等级",
                    "Scholarly project index and navigation source; it is not itself "
                    "a primary paleographic catalog, an inscription transcription, "
                    "or a confirmed decipherment.",
                ),
                "",
                "## Indexed Research Relations / 索引的研究关系",
                *bullet(
                    "Datasets / 数据集",
                    "The README links HUST-OBC, EVOBC, OBIMD, Oracle-MNIST, "
                    "GEVOBC/GBEDOBC, and other datasets with task and access links. "
                    "Each dataset retains its own source object and rights review.",
                ),
                *bullet(
                    "Primary paper routes / 论文主路线",
                    "The index provides official routes for HUST-OBC, EVOBC, and the "
                    "graph-based evolutionary dataset, including DOI or publisher "
                    "links where available. These links support discovery; their "
                    "claims are reviewed in the corresponding source objects.",
                ),
                *bullet(
                    "AI task categories / AI 任务分类",
                    "The README groups recognition, detection, retrieval, rejoining, "
                    "restoration, generation, interpretation, and decipherment. A "
                    "task label is not evidence that a model produced a scholarly "
                    "reading.",
                ),
                *bullet(
                    "Human research resources / 人类研究资源",
                    "The online-resource section points to YinQiWenYuan, Xiaoxuetang, "
                    "Guo Xue Da Shi, museum collections, and other databases. These "
                    "routes must be opened for object, catalog, and rights evidence.",
                ),
                "",
                "## Paper And Citation Examples / 论文与引用示例",
                *bullet(
                    "EVOBC route / EVOBC 路线",
                    "The index cites Guan et al., An Open Dataset for the Evolution "
                    "of Oracle Bone Characters, arXiv:2401.12467; the EVOBC source "
                    "object holds the detailed paper and dataset review.",
                ),
                *bullet(
                    "HUST-OBC route / HUST-OBC 路线",
                    "The index cites Wang et al., Scientific Data 11, 976 (2024), "
                    "DOI 10.1038/s41597-024-03807-x; the HUST source object holds "
                    "the dataset scale, process, and license review.",
                ),
                *bullet(
                    "OBSD route / OBSD 路线",
                    "The index lists Guan et al., Deciphering Oracle Bone Language "
                    "with Diffusion Models, ACL 2024, DOI 10.18653/v1/2024.acl-long.831. "
                    "Its model outputs remain technical evidence, not accepted "
                    "readings.",
                ),
                "",
                "## Limits, Disputes, And Rights / 限制、争议与权利",
                *bullet(
                    "Index drift / 索引漂移",
                    "A project README changes as new papers, datasets, links, and "
                    "task labels are added. Preserve the local checksum and compare "
                    "the snapshot with the current page before citing it.",
                ),
                *bullet(
                    "Underlying source boundary / 下游来源边界",
                    "Open-Oracle does not transfer the rights, provenance, catalog "
                    "identity, or scholarly status of any linked dataset. Those "
                    "questions remain in each source object.",
                ),
                *bullet(
                    "Decipherment boundary / 释读边界",
                    "Words such as decipherment, interpretation, benchmark, and "
                    "reasoning describe indexed research tasks; they do not turn an "
                    "AI output into a confirmed academic conclusion.",
                ),
                *bullet(
                    "Rights / 权利",
                    "The repository welcomes contributions but does not clear every "
                    "linked asset. Review each dataset, paper, website, image, and "
                    "third-party material before public redistribution.",
                ),
                "",
                "## Concrete Bibliography Checks / 具体文献核查",
                "- Verify the local README checksum, repository revision, and current",
                "  page maintenance date before citing an index entry.",
                "- Follow every paper, code, data, museum, and database link and",
                "  record the target source object rather than copying a summary.",
                "- Compare the Open-Oracle entries for HUST-OBC, EVOBC, OBIMD, and",
                "  GBEDOBC with their independent literature files.",
                "- Record changed links, missing licenses, alternate labels, and",
                "  disputed task descriptions as concrete review questions.",
                "- 复核本地 README checksum、仓库版本和当前页面维护日期，",
                "  再引用索引条目。",
                "- 逐一打开论文、代码、数据、博物馆和数据库链接，将目标",
                "  来源写入对应来源对象，不复制摘要作为结论。",
                "- 将 Open-Oracle 中的 HUST-OBC、EVOBC、OBIMD 和 GBEDOBC",
                "  与各自独立文献档案逐项核对。",
                "- 把失效链接、缺失许可、替代标签和争议任务描述写成",
                "  具体待复核问题。",
                "",
            ]
        )
    ...
    elif source["source_id"] == "src-ihp-oracle-rubbings":
        lines.extend(
            [
                "## Institutional Rubbing Database / 机构拓片数据库",
                *bullet(
                    "Institution and function / 机构与功能",
                    "The Fu Ssu Nien Library Digital Archives Project describes this "
                    "as a database of oracle-bone rubbings held by the Institute of "
                    "History and Philology, with cross-reference to archaeological "
                    "objects and images.",
                ),
                *bullet(
                    "Work history / 建设与维护",
                    "The page records a 2004-2009 project led by Yuan Guohua and says "
                    "the rubbing database results were transferred to Fu Ssu Nien "
                    "Library for continuing maintenance from 2009.",
                ),
                *bullet(
                    "Evidence level / 证据等级",
                    "Institutional collection database and image-provenance route; a "
                    "record still needs object, catalog, image, and rights review "
                    "before entering a public dossier.",
                ),
                "",
                "## Collection Scope And Human Use / 收藏范围与人工使用",
                *bullet(
                    "Collection scope / 收藏范围",
                    "The official overview reports more than 40,000 rubbings, divided "
                    "between rubbings from archaeological excavations and rubbings "
                    "obtained through later acquisitions.",
                ),
                *bullet(
                    "Searchable records / 可检索记录",
                    "The page reports 21,556 digitized records searchable through the "
                    "database. This is an access count, not a count of locally "
                    "downloaded or rights-cleared images.",
                ),
                *bullet(
                    "Bibliographic examples / 著录示例",
                    "The overview names Iron Cloud Collection of Four Hundred Turtle "
                    "Shells and several Xiaotun second-volume illustration sections, "
                    "among other rubbing publications. Each title needs edition and "
                    "plate-level checking.",
                ),
                *bullet(
                    "Object link / 实物关联",
                    "Collection number is the cross-reference key between rubbing "
                    "records and archaeological artifacts or images. It is a route "
                    "key, not a universal character or inscription identity.",
                ),
                "",
                "## Human Rubbing Review Order / 人工拓片复核顺序",
                *bullet(
                    "Open the rubbing / 先看拓片",
                    "Review the visible rubbing and its record before reading any "
                    "modern label, OCR, or model-derived category.",
                ),
                *bullet(
                    "Cross-check object / 核对实物",
                    "Use the collection number to open the linked artifact and image, "
                    "then record collection, object type, physical support, and any "
                    "available catalog or excavation note separately.",
                ),
                *bullet(
                    "Research fields / 研究字段",
                    "A human dossier should preserve rubbing path, collection number, "
                    "catalog or plate reference, image quality, visible damage, text "
                    "context, and missing provenance as separate evidence fields.",
                ),
                "",
                "## Rights, Access, And Disputes / 权利、访问与争议",
                *bullet(
                    "Rights notice / 权利提示",
                    "The official page directs users to follow the site copyright "
                    "notice and rules for citation or reproduction. Local status "
                    "remains metadata_only_until_verified.",
                ),
                *bullet(
                    "Access boundary / 访问边界",
                    "The local package contains only a lightweight overview capture. "
                    "Do not treat the 21,556 searchable records as local image files "
                    "or as permission to bulk download them.",
                ),
                *bullet(
                    "Collection distinction / 收藏区分",
                    "Excavation-derived and later-acquired rubbings have different "
                    "provenance questions. Keep those categories visible rather than "
    ...
    elif source["source_id"] == "src-cambridge-hopkins":
        lines.extend(
            [
                "## Official Finding-List Evidence / 官方目录证据",
                *bullet(
                    "Primary page / 主要页面",
                    "Cambridge University Library, Finding List for the Hopkins "
                    "Collection of Chinese Oracle Bones: "
                    "https://www.lib.cam.ac.uk/collections/departments/"
                    "chinese-collections/chinese-collections-te-cang-yu-zhuan-cang/"
                    "finding-list",
                ),
                *bullet(
                    "Collection scale / 馆藏规模",
                    "The page reports a grand total of 609 objects in the "
                    "classified finding list; the period and topic table is "
                    "metadata for navigation, not an archaeological identity "
                    "decision.",
                ),
                *bullet(
                    "Digitized access / 数字访问",
                    "The page says the Hopkins bones have been digitised and that "
                    "50 bones are available in 3D through the Chinese Works page. "
                    "This is an access route, not local image or rights clearance.",
                ),
                *bullet(
                    "Finding-list fields / 目录字段",
                    "Rows expose period, group, Yingguo, CUL, Chalfant, and Heji "
                    "references. Asterisks, missing numbers, and notes such as "
                    "no reverse must remain source observations for review.",
                ),
                "",
                "## Bibliographic Key And Human Use / 著录键与人工使用",
                *bullet(
                    "Reference key / 著录键",
                    "The page defines c as Cambridge University Library, h as "
                    "Chalfant, j as Jiaguwen Heji, and y as Yingguo suo cang "
                    "jiagu ji. These keys route a reviewer to other catalogs; "
                    "they do not prove that records are the same object.",
                ),
                *bullet(
                    "Human dossier fields / 人类档案字段",
                    "Retain the finding-list row, catalog references, period, group, "
                    "plate or image route, collection object, findspot, text, "
                    "and page-level evidence separately.",
                ),
                *bullet(
                    "Topic classification / 主题分类",
                    "The table includes topics such as ancestors, warfare, hunting, "
                    "sacrifice, and divinatory techniques. Preserve these as the "
                    "source's classification labels, not as a new interpretation "
                    "of any inscription.",
                ),
                "",
                "## Citation Relations And Limits / 引用关系与限制",
                *bullet(
                    "Citation routes / 引用路线",
                    "The key points to Chalfant's Hopkins catalog, Jiaguwen Heji, "
                    "and Yingguo suo cang jiagu ji. Each cited entry needs its own "
                    "edition, page, plate, image, and rights review.",
                ),
                *bullet(
                    "Unresolved notes / 未决注记",
                    "Notes such as no reverse, no CUL number, joins, and missing "
                    "Heji values are not silently corrected. Record the source row "
                    "and make the next catalog query explicit.",
                ),
                *bullet(
                    "Interpretive boundary / 释读边界",
                    "The finding list is a collection and catalog route. It does "
                    "not supply a complete transcription, accepted reading, "
                    "formal inscription identity, or decipherment conclusion.",
                ),
                *bullet(
                    "Rights and access / 权利与访问",
                    "The linked Cambridge Digital Library and 3D materials require "
                    "separate image, IIIF, citation, and permission checks. Local "
                    "status remains metadata_only_until_verified.",
                ),
                "",
                "## Concrete Bibliography Checks / 具体文献核查",
                "- Verify the current finding-list page, local HTML checksum, and",
                "  source download log before reusing a row or count.",
                "- Open the cited CUL, Chalfant, Heji, and Yingguo records and",
                "  record edition, page, plate, image, and object evidence.",
                "- Follow the Chinese Works and 3D route only after checking image",
                "  rights and whether the item matches the catalog reference.",
                "- Keep period, group, topic, join, and missing-number notes as",
                "  source metadata and concrete review questions.",
                "- 复核当前目录页、本地 HTML checksum 和下载日志后，才能复用",
                "  单行目录或统计数字。",
                "- 打开 CUL、Chalfant、合集和英国所藏甲骨集的对应记录，记录",
                "  版本、页码、图版、图像和实物证据。",
                "- 进入 Chinese Works 或 3D 路线前，先核对图像权利和对象是否",
                "  与著录引用相符。",
                "- 把时期、组别、主题、拼合和缺号注记保留为来源元数据及具体",
                "  待查问题。",
                "",
            ]
        )
    elif source["source_id"] == "src-ihp-museum-oracle-bones":
        lines.extend(
            [
                "## Museum Collection Description / 博物馆馆藏说明",
                *bullet(
                    "Institutional context / 机构背景",
                    "The Museum of the Institute of History and Philology presents "
                    "oracle bones as a collection formed through the Institute's "
                    "research and scientific excavation history.",
                ),
                *bullet(
                    "Excavation history / 发掘历史",
                    "The official page states that large-scale scientific excavation "
                    "began after the Institute was established in 1928, and that "
                    "excavations at Xiaotun brought forth more than 25,000 oracle "
                    "bone pieces.",
                ),
                *bullet(
                    "Evidence level / 证据等级",
                    "Museum collection and selected-object catalog route; each object "
                    "page is human evidence for material, catalog label, image, and "
                ...
                *bullet(
                    "Object types / 器物类型",
                    "The collection page includes ox scapulae, tortoise carapaces, "
                    "plastrons, animal bones, skulls, antler objects, and joined or "
                    "fragmentary pieces. Preserve the material type before studying "
                    "the inscription.",
                ),
                *bullet(
                    "Catalog-style labels / 著录式标签",
                    "Selected entries show labels such as Jia Bian 3333+3361, Bing Bian "
                    "0008, Yi Bian 3330+5281, and Ping 0086. These labels are museum "
                    "or catalog routes and require source-book expansion.",
                ),
                *bullet(
                    "Joined objects / 拼合实物",
                    "Plus signs in entries such as Jia Bian 3333+3361 or Yi Bian "
                    "3330+5281+Yi Bian buyi 4936 indicate a joined or cross-referenced "
                    "catalog route; inspect the object page before treating it as a "
                    "physical reconstruction.",
                ),
                *bullet(
                    "Human dossier fields / 人类档案字段",
                    "For each object, retain object type, material, catalog label, "
                    "image path, visible damage, inscription placement, collection, "
                    "provenance, and missing findspot or period evidence separately.",
                ),
                "",
                "## Archaeological And Bibliographic Boundaries / 考古与书目边界",
                *bullet(
                    "Material before interpretation / 先实物后释读",
                    "Open the museum object image and catalog label first, then compare "
                    "rubbing, Heji, old catalog, and inscription records. The museum "
                    "page alone does not supply full OCR or a scholarly reading history.",
                ),
                *bullet(
                    "Collection versus findspot / 馆藏与出土地",
                    "A museum collection page establishes a collection route, not that "
                    "the current holding is the original findspot. Keep excavation, "
                    "acquisition, and holding evidence distinct.",
                ),
                *bullet(
                    "Cross-source links / 跨来源链接",
                    "Use catalog labels to seek related Heji, rubbing, and IHP database "
                    "records, but require agreement of object and image evidence before "
                    "forming a cross-source object link.",
                ),
                "",
                "## Rights, Access, And Review / 权利、访问与复核",
                *bullet(
                    "Museum policy / 博物馆规则",
                    "The official page provides museum policy and photograph-permission "
                    "routes. Local status remains metadata_only_until_verified until "
                    "those rules and each image credit are checked.",
                ),
                *bullet(
                    "Local package boundary / 本地包边界",
                    "The committed package is a lightweight collection-page capture. "
                    "It does not prove complete collection coverage or grant permission "
                    "to copy the selected images.",
                ),
                *bullet(
                    "Disputed interpretation / 争议解释",
                    "Object type, catalog label, period, inscription identity, and "
                    "reading must remain separate review fields; no museum label alone "
                    "resolves a paleographic dispute.",
                ),
                "",
                "## Concrete Bibliography Checks / 具体文献核查",
