#!/usr/bin/env python3
"""Build a first-wave handoff scaffold for Xiaoxuetang capture-result review tasks."""
...
TARGET_WAVE_ID = "xxt-followup-capture-assignment-wave-001"
TARGET_FAMILY_ID = "xxt_jgw_tls_access_boundary_followup"
STATUS = "draft_capture_wave_handoff_scaffold_not_started"
RESEARCH_BOUNDARY = "xxt_followup_capture_wave_handoff_scaffold_not_scholarship"
OUTPUT_SCOPE = "xiaoxuetang_followup_capture_wave_handoff_scaffold_only"
...
def _handoff_item(item: dict[str, object], wave: dict[str, object]) -> dict[str, object]:
    ...
    return {
        "handoff_item_id": str(item["assignment_plan_item_id"]).replace("assignment", "handoff"),
        "assignment_wave_id": wave["assignment_wave_id"],
        "assignment_plan_item_id": item["assignment_plan_item_id"],
        "capture_route_id": item["capture_route_id"],
        "capture_result_id": item["capture_result_id"],
        ...
        "route_status_snapshot": item["route_status_snapshot"],
        "assignment_status": item["assignment_status"],
        "handoff_status": "ready_for_xxt_capture_followup_not_started",
        ...
        "caution": (
            "This handoff row only opens the first reviewed metadata-only Xiaoxuetang capture follow-up wave for later manual-browser or institutional-export review. "
...
def build_wave_handoff_scaffold(assignment_plan: dict[str, object]) -> dict[str, object]:
    ...
    target_waves = [wave for wave in waves if wave.get("assignment_wave_id") == TARGET_WAVE_ID and wave.get("followup_family_id") == TARGET_FAMILY_ID]
    if len(target_waves) != 1:
        raise ValueError("expected exactly one first Xiaoxuetang capture assignment wave")
    wave = target_waves[0]

    target_ids = set(wave.get("assignment_plan_item_ids", []))
    items = [item for item in list(assignment_plan.get("assignment_items", [])) if item.get("assignment_plan_item_id") in target_ids]
    ...
    unique_route_files = _unique_sorted([route_file for item in handoff_items for route_file in item["route_files_to_open"]])
    unique_review_sections = _unique_sorted([section for item in handoff_items for section in item["required_review_sections"]])
    unique_next_checks = _unique_sorted([check for item in handoff_items for check in item["required_next_checks"]])

    return {
        "context_pack_id": "ai-context-xiaoxuetang-followup-capture-wave-handoff-001",
        "title": "Xiaoxuetang Follow-up Capture Wave Handoff Scaffold",
        "title_zh": "小学堂 follow-up capture wave handoff scaffold",
        "status": STATUS,
        "updated_at": UPDATED_AT,
        "generated_from": [ASSIGNMENT_PLAN.as_posix(), REVIEW_ROUTE_SUMMARY.as_posix(), ROUTE_PACK.as_posix()],
        "purpose": (
            "Open the first Xiaoxuetang capture-result assignment wave from the 088 plan for future JGW manual-browser or institutional-export review. "
            "This scaffold only lists handoff items and required files; it does not collect evidence, assign an owner, decide rights, confirm catalogs or collections, or make decipherment claims."
        ),
        "purpose_zh": "从 088 计划中打开第一波小学堂 capture-result assignment wave，供后续 JGW 人工浏览器或机构导出复核使用。本脚手架只列出 handoff items 和必开文件，不采证，不分配 owner，不做权利判断，不确认著录或馆藏，也不做释读判断。",
        "research_boundary": RESEARCH_BOUNDARY,
        "output_scope": OUTPUT_SCOPE,
        ...
        "handoff_scope": {
            ...
            "capture_result_ids": wave["capture_result_ids"],
            ...
            "human_review_status": wave["human_review_status"],
        ...
        "coverage": {
            ...
            "capture_result_count": len(handoff_items),
            "followup_family_count": 1,
            "target_source_count": len({item["target_source_id"] for item in handoff_items}),
            "route_file_reference_count": sum(len(item["route_files_to_open"]) for item in handoff_items),
            ...
            "followup_family_counts": {TARGET_FAMILY_ID: len(handoff_items)},
            "target_source_counts": dict(sorted(Counter(str(item["target_source_id"]) for item in handoff_items).items())),
            ...
            "human_review_status_counts": _status_counts(handoff_items, "human_review_status"),
            "catalog_claim_status_counts": _status_counts(handoff_items, "catalog_claim_status"),
            "collection_match_status_counts": _status_counts(handoff_items, "collection_match_status"),
            "rights_decision_status_counts": _status_counts(handoff_items, "rights_decision_status"),
        },
        ...
        "agent_use_rules": [
            "Use this handoff scaffold only to open the first Xiaoxuetang capture-result follow-up wave.",
            "Open the 089 handoff row, 088 plan item, 087 review summary, 086 route pack, capture-result row, and all route files before recording any evidence.",
        ...
        "agent_use_rules_zh": [
            "本 handoff scaffold 只能用于打开第一波小学堂 capture-result follow-up 路线。",
            "记录任何证据前，必须先打开 089 handoff row、088 plan item、087 review summary、086 route pack、capture-result 行和全部 route files。",
            "在带来源标记的 follow-up 证据于其他位置记录前，每行都必须保持 not_collected、not_decided 和 no_claim。",
