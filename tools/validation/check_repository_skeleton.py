...
def check_hust_obc_undeciphered_candidates(root: Path) -> list[str]:
    ...
    if len(materialized) != 3600:
        issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} should materialize first 3600 rows")
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if index <= 3600:
            if not packet_path:
                issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} first 3600 rows need packet paths")
            elif not (root / packet_path).exists():
                issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} missing packet: {packet_path}")
        elif packet_path:
            issues.append(f"{HUST_OBC_UNDECIPHERED_CANDIDATE_INDEX} rows after first 3600 should not claim packets")
