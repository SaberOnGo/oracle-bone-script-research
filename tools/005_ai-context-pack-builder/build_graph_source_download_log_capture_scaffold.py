#!/usr/bin/env python3
"""Build a download-log evidence capture scaffold from the second handoff wave."""
...
FIELDNAMES = [
    ...
    "source_download_log_row_status",
...
def _capture_row(index: int, item: dict[str, object]) -> dict[str, str]:
    return {
        "capture_row_id": f"graph-source-evidence-download-log-capture-{index:03d}",
        ...
        "source_download_log_path": SOURCE_DOWNLOAD_LOG_PATH,
        ...
        "caution": (
            "Empty capture scaffold for download-log evidence only. Fill this "
            "row only after opening the download log and cited route files; do "
            "not infer rights from download success, and do not use this row as "
            "collected evidence, a checksum review, a size review, source "
...
def build_capture_scaffold(handoff_scaffold: dict[str, object]) -> list[dict[str, str]]:
    ...
    if any(row["target_evidence_section"] != "download_log" for row in rows):
        raise ValueError("capture scaffold only supports download_log rows")
    return rows
