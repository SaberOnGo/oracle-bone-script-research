...
`build_hust_obc_undeciphered_candidate_index.py` 会从本地临时区或外部归档中的 HUST-OBC figshare 原始 zip 读取目录结构，写出 metadata-only 的 `obs-unk-*` 未释读候选索引，并按配置生成每 100 条一组的候选 packet bucket。目前已落实 `obs-unk-000001..obs-unk-002000`。它只记录来源图片路径和数量；原始图片文件保留在普通 Git 之外。
