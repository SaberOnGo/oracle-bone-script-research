#!/usr/bin/env python3
"""Build a route pack for missing-evidence review outcome scaffolds.

The route pack indexes the 155 outcome scaffold rows so a later reviewer can
open the right source files before filling human-reviewed outcomes. It does
...
DEFAULT_OUTPUT = STAT_DIR / "156_source-pipeline-phase-action-missing-evidence-review-outcome-route-pack.json"

UPDATED_AT = "2026-06-19"
ROUTE_STATUS = "not_started"
AUTOMATION_BOUNDARY = "routing_only_no_missing_evidence_outcome_capture"
RESEARCH_BOUNDARY = "source_pipeline_phase_action_missing_evidence_review_outcome_route_pack_not_scholarship"
CAUTION = (
    "This source pipeline missing-evidence review outcome route pack is routing-only. "
...
def route_from_row(index: int, row: dict[str, str]) -> dict[str, object]:
    return {
        "route_id": f"source-pipeline-missing-evidence-review-outcome-route-{index:03d}",
        ...
        "outcome_scaffold_path": SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_OUTCOME_SCAFFOLD.as_posix(),
...
def build_route_pack(outcome_rows: list[dict[str, str]]) -> dict[str, object]:
    ...
    return {
        "route_pack_id": "source-pipeline-missing-evidence-review-outcome-route-pack-001",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build missing-evidence review outcome route pack.")
    parser.add_argument("--outcome-scaffold", default=str(SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_OUTCOME_SCAFFOLD))
