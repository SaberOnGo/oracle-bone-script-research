...
MATERIAL_VISUAL_OBSERVATIONS = {
    ...
    "obs-char-001401": (
        "A tall open form has an angular upper cap, a central body, and lower prongs.",
        "高大开放形体有角状上盖、中央主体和下部分叉。",
    ),
    "obs-char-001402": (
        "A broad upper frame stands above several narrow branching lower strokes.",
        "宽大上部框形位于数条狭长下部分枝笔画之上。",
    ),
    "obs-char-001403": (
        "A small upper frame leads into a narrow lower cluster with a long curve.",
        "小型上部框形连接狭长下部笔画群，旁有长弧线。",
    ),
    "obs-char-001404": (
        "A horizontal upper frame sits above two separated symmetrical branch clusters.",
        "水平上部框形位于两个分离对称分枝群之上。",
    ),
    "obs-char-001405": (
        "A broad upper frame stands above two small lower enclosures and short marks.",
        "宽大上部框形位于两个小型下部框形和短笔画之上。",
    ),
    "obs-char-001406": (
        "A horizontal upper frame sits above two separated symmetrical branch clusters.",
        "水平上部框形位于两个分离对称分枝群之上。",
    ...
    "obs-char-001407": (
        "A chain of rounded left loops is paired with a pointed right enclosure.",
        "一串圆拱左侧环形与尖状右侧框形相邻。",
    ),
    "obs-char-001408": (
        "A small upper frame leads to a diagonal central stroke and lower branches.",
        "小型上部框形连接中央斜笔和下部分枝。",
    ),
    "obs-char-001409": (
        "Two tall stems have short upper caps, inner branches, and long lower strokes.",
        "两条高大竖干有短上盖、内部枝笔和长下部笔画。",
    ),
    "obs-char-001410": (
        "A large leaf-like enclosure contains a smaller rounded inner form.",
        "大型叶状框形内部有较小圆拱形体。",
    ),
    "obs-char-001411": (
        "A compact knot has an upper enclosure, a central crossing, and lower branches.",
        "紧凑笔画结有上部框形、中央交叉笔和下部分枝。",
    ),
    "obs-char-001412": (
        "A narrow upright form has a dark upper stroke and a long irregular lower stem.",
        "狭长直立形体有深色上部笔画和长不规则下部竖干。",
    ),
    "obs-char-001413": (
        "Three stacked angular horizontal bands are joined by short central strokes.",
        "三个叠置角状水平带由短中央笔画连接。",
    ),
    "obs-char-001414": (
        "A narrow composite has a small upper loop and a long zigzagging lower stroke.",
        "狭长组合形体有小型上部环形和长下部曲折笔画。",
    ),
    "obs-char-001415": (
        "A large stacked form has rounded upper bands, a central enclosure, and lower legs.",
        "大型叠置形体有圆拱上部带、中央框形和下部分叉笔。",
    ),
    "obs-char-001416": (
        "A small narrow form has an upper rectangular cap and a long descending curve.",
        "小型狭长形体有上部矩形盖和长下行弧线。",
    ),
    "obs-char-001417": (
        "A rounded upper enclosure has side prongs and a pointed lower taper.",
        "圆拱上部框形有侧部分叉和尖状下部收束。",
    ),
    "obs-char-001418": (
        "A large oval enclosure contains a small inner form above a long branching stem.",
        "大型椭圆框形内部有小型形体，下接长分枝竖干。",
    ),
    "obs-char-001419": (
        "Two vertical forms combine a curved left stem with three right side marks.",
        "两个竖向形体组合为弧形左竖干和三个右侧笔画。",
    ),
    "obs-char-001420": (
    ...
    "obs-char-001461": (
        "A compact upper cluster leads into a narrow stem with several lower branches.",
        "紧凑上部笔画群连接狭长竖干，下面有数条分枝。",
    ),
    "obs-char-001462": (
        "An open left frame is paired with a long right diagonal and side branches.",
        "开放左侧框形与长右侧斜笔及侧部分枝相邻。",
    ),
    "obs-char-001463": (
        "A symmetrical pointed upper cluster sits above a dense lower stem.",
        "对称尖状上部笔画群位于密集下部竖干之上。",
    ),
    "obs-char-001464": (
        "A large composite has a rounded upper enclosure, a long stem, and lower legs.",
        "大型组合形体有圆拱上部框形、长竖干和下部分叉。",
    ),
    "obs-char-001465": (
        "A faint curved cluster is formed by several short parallel strokes.",
        "浅淡弧形笔画群由数条短平行笔画组成。",
    ),
    "obs-char-001466": (
        "A faint compact cluster has a pointed upper contour and lower branches.",
        "浅淡紧凑笔画群有尖状上部轮廓和下部分枝。",
    ),
    "obs-char-001467": (
        "A closed oval enclosure contains several internal crossing divisions.",
        "封闭椭圆框形内部有数条交叉隔分笔画。",
    ),
    "obs-char-001468": (
        "A diagonal central band has an upper branch and a lower right stroke.",
        "中央斜向带状形有上部分枝和右侧下部笔画。",
    ),
    "obs-char-001469": (
        "A small upper loop leads to a narrow stem with a right diagonal branch.",
        "小型上部环形连接狭长竖干，旁有右侧斜向分枝。",
    ),
    "obs-char-001470": (
        "A compact dense cluster combines stacked angular forms and a central diagonal.",
        "紧凑密集笔画群组合叠置角状形体和中央斜笔。",
    ),
    "obs-char-001471": (
        "A narrow upper enclosure has side branches above a long lower stroke.",
        "狭长上部框形有侧部分枝，下接长下部笔画。",
    ),
    "obs-char-001472": (
        "A rounded upper enclosure leads into a long diagonal lower stem.",
        "圆拱上部框形连接长下部斜向竖干。",
    ),
    "obs-char-001473": (
        "A stacked central form has a broad upper bar, lower curves, and detached marks.",
        "叠置中央形体有宽大上横笔、下部弧线和分离短笔。",
    ),
    "obs-char-001474": (
        "A compact composite has divided upper bars, a small middle loop, and a stem.",
        "紧凑组合形体有隔分上横笔、小型中部环形和竖干。",
    ),
    "obs-char-001475": (
        "Two faint adjacent forms repeat a narrow stem and small side branches.",
        "两个浅淡相邻形体重复狭长竖干和小型侧部分枝。",
    ),
    "obs-char-001476": (
        "A tall simple stem has a broad upper fork and lower side branches.",
        "高大简单竖干有宽大上部分叉和下部侧枝。",
    ),
    "obs-char-001477": (
        "A peaked upper stem sits above a small divided enclosure and two prongs.",
        "尖拱上部竖干位于小型隔分框形和两个分叉之上。",
    ),
    "obs-char-001478": (
        "A narrow tall cluster has a branching left edge and detached right marks.",
        "狭长高大笔画群有分枝左缘和分离右侧短笔。",
    ),
    "obs-char-001479": (
        "A simple pointed upper loop leads into two long descending strokes.",
        "简单尖状上部环形连接两条长下行笔画。",
    ),
    "obs-char-001480": (
    ...
    "obs-char-001541": (
        "A composite has an upper branching cluster, a row of small enclosures, and detached lower marks.",
        "组合形体有上部分枝笔画群、一排小型框形和分离的下部标记。",
    ),
    "obs-char-001542": (
        "Two stacked enclosures show repeated upper prongs and a broad lower rectangular contour.",
        "两个上下叠置框形有重复上部分叉和宽大的下部矩形轮廓。",
    ),
    "obs-char-001543": (
        "Sparse strokes surround a central vertical branch, with several detached short marks.",
        "稀疏笔画围绕中央分枝竖笔，周围还有数个分离短笔。",
    ),
    "obs-char-001544": (
        "A dense curved cluster has a central crossing and several sweeping side strokes.",
        "密集弧形笔画群有中央交叉和数条舒展侧部笔画。",
    ),
    "obs-char-001545": (
        "A small dark mark combines short crossing diagonals around a central core.",
        "小型深色标记以中央核心结合数条短交叉斜笔。",
    ),
    "obs-char-001546": (
        "A composite has a rounded upper enclosure, a pointed lower base, and a long right contour.",
        "组合形体有圆折上部框形、尖状下部底形和右侧长轮廓。",
    ),
    "obs-char-001547": (
        "Two separated forms show a small pointed loop on the left and a branching narrow form on the right.",
        "两个分离形体左侧有小型尖顶环，右侧有狭长分枝形体。",
    ),
    "obs-char-001548": (
        "A very faint form has a short upper bar, a small central enclosure, and lower vertical marks.",
        "极浅淡形体有短上横笔、小型中央框形和下部竖向笔画。",
    ),
    "obs-char-001549": (
        "A sparse composite pairs two curved left strokes with a small angular right cluster.",
        "稀疏组合形体将两条左侧弧笔与右侧小型折角笔画群相配。",
    ),
    "obs-char-001550": (
        "A large dark form has a rounded upper enclosure, a broad middle body, and long lower strokes.",
        "大型深色形体有圆折上部框形、宽大中部主体和长下部笔画。",
    ),
    "obs-char-001551": (
        "A large rectangular grid contains two upper loops, internal crossbars, and a double lower base.",
        "大型矩形格状框形包含两个上部环形、内部横笔和双层下部底笔。",
    ),
    "obs-char-001552": (
        "Two narrow adjacent forms show a branching curved left contour and short right crossbars.",
        "两个狭长相邻形体有分枝左侧弧形轮廓和右侧短横笔。",
    ),
    "obs-char-001553": (
        "A broad U-shaped contour stands on two short legs above a horizontal base.",
        "宽大的 U 形轮廓由两条短支脚承托，底部有横笔。",
    ),
    "obs-char-001554": (
        "A pointed upper enclosure leads into a narrow branching lower stroke.",
        "尖顶上部框形连接狭长分枝下部笔画。",
    ),
    "obs-char-001555": (
        "A bold diamond-like enclosure contains several rounded inner openings.",
        "醒目的菱形框形包含数个圆折内部开口。",
    ),
    "obs-char-001556": (
        "Several small angular enclosures are arranged around a long descending diagonal.",
        "数个小型折角框形围绕长下斜笔排列。",
    ),
    "obs-char-001557": (
        "A very faint small form has an upper cross, a central loop, and a lower diagonal.",
        "极浅淡的小型形体有上部交叉、中央环形和下部斜笔。",
    ),
    "obs-char-001558": (
        "A dense branching cluster has several upright strokes and a short horizontal foot.",
        "密集分枝笔画群有数条直立笔画和短横向底笔。",
    ),
    "obs-char-001559": (
        "A dark composite has an upper branch, two lower openings, and crossed bottom strokes.",
        "深色组合形体有上部分枝、两个下部开口和交叉底部笔画。",
    ),
    "obs-char-001560": (
        "A complex composite combines an upper loop, a middle enclosure with an inner V, and a lower zigzag.",
        "复杂组合形体结合上部环形、含内部 V 形的中部框形和下部曲折形。",
    ),
    "obs-char-001561": (
        "A dark tall form has upper loops, a central opening, and branching pointed lower strokes.",
        "深色高大形体有上部环形、中央开口和分枝尖状下部笔画。",
    ),
    "obs-char-001562": (
        "A sparse form has a long right vertical, two small left loops, and a lower bowl-like mark.",
        "稀疏形体有右侧长竖笔、两个左侧小环和下部盆状标记。",
    ),
    "obs-char-001563": (
        "A heavily degraded image preserves two dark irregular clusters with only partial contours visible.",
        "严重退化图像保留两个深色不规则笔画群，仅能看见部分轮廓。",
    ),
    "obs-char-001564": (
        "A narrow pointed enclosure contains crossing inner strokes and a curled lower mark.",
        "狭长尖顶框形包含交叉内部笔画和卷曲下部标记。",
    ),
    "obs-char-001565": (
        "Two adjacent forms show a segmented rounded loop on the left and a smaller curved form on the right.",
        "两个相邻形体左侧有分段圆折环，右侧有较小弧形形体。",
    ),
    "obs-char-001566": (
        "Several forked marks surround a central stem with two small rounded openings.",
        "数个分叉标记围绕中央竖干，中央有两个小型圆折开口。",
    ),
    "obs-char-001567": (
        "A tall winding contour has an upper angled section, side branches, and a lower diamond loop.",
        "高大曲折轮廓有上部斜向段、侧部分枝和下部菱形环。",
    ),
    "obs-char-001568": (
        "A very faint small form preserves a shallow upper curve and a short lower crossbar.",
        "极浅淡的小型形体保留浅弧形上笔和短下部横笔。",
    ),
    "obs-char-001569": (
        "A faint narrow cluster shows several short branching and angular strokes.",
        "浅淡狭长笔画群显示数条短分枝和折角笔画。",
    ),
    "obs-char-001570": (
        "A composite has an upper rounded enclosure, side marks, and a dense lower branching cluster.",
        "组合形体有上部圆折框形、侧部标记和密集下部分枝笔画群。",
    ),
    "obs-char-001571": (
        "Two stacked forms show an open rectangular upper contour and a broad pointed lower mark.",
        "两个上下叠置形体有开放矩形上部轮廓和宽大的尖状下部标记。",
    ),
    "obs-char-001572": (
        "A faint small cluster preserves a central crossing with short parallel strokes.",
        "浅淡小型笔画群保留中央交叉和短平行笔画。",
    ),
    "obs-char-001573": (
        "Two adjacent forms show a dense branching left contour and a separate narrow right stroke.",
        "两个相邻形体左侧有密集分枝轮廓，右侧有分离的狭长笔画。",
    ),
    "obs-char-001574": (
        "A very small form combines a short upper bar, a middle band, and a lower pointed mark.",
        "极小形体结合短上横笔、中部带状结构和下部尖状标记。",
    ),
    "obs-char-001575": (
        "A tall composite has upper short strokes, a central diagonal body, a lower bowl, and a detached side mark.",
        "高大组合形体有上部短笔、中央斜向主体、下部盆状形和分离侧部标记。",
    ),
    "obs-char-001576": (
        "Two separated forms pair a branching left cluster with a small rounded upper enclosure on the right.",
        "两个分离形体将左侧分枝笔画群与右侧小型圆折上部框形相配。",
    ),
    "obs-char-001577": (
        "A broad bent enclosure has segmented upper strokes and a rectangular lower extension.",
        "宽大的弯折框形有分段上部笔画和矩形下部延伸。",
    ),
    "obs-char-001578": (
        "A stacked form has an upper crossbar, a narrow middle stem, and a lower rounded enclosure.",
        "上下叠置形体有上部横笔、狭长中部竖干和下部圆折框形。",
    ),
    "obs-char-001579": (
        "Two large adjacent forms show branching left strokes and a right enclosure with long side branches.",
        "两个相邻大型形体左侧有分枝笔画，右侧有带长侧部分枝的框形。",
    ),
    "obs-char-001580": (
    ...
    "obs-char-001182": (
        "A narrow upright form has a forked upper stroke, a short detached right mark, and a rounded lower enclosure.",
        "狭长直立形体有分叉上笔、右侧短分离痕迹和圆弧形下部框廓。",
    ),
    "obs-char-001183": (
        "A bold upright form combines a broad upper cross-stroke, a central vertical, and several lower horizontal turns.",
        "粗重直立形体结合宽大上部横笔、中央竖笔和数个下部横向转折。",
    ),
    "obs-char-001184": (
        "A small narrow form has a short upper bar, a central loop-like enclosure, and a rounded lower hook.",
        "小型狭长形体有短上部横笔、中央环状框廓和圆弧形下部钩笔。",
    ),
    "obs-char-001185": (
        "A broad upper bar joins a central upright and three long hanging strokes with short side branches.",
        "宽大的上部横笔连接中央直立笔和三条带短侧枝的长下垂笔。",
    ),
    "obs-char-001186": (
        "A narrow sloping form has several short curved bands and a long thin descending lower stroke.",
        "狭长斜向形体有数条短弯曲带和一条细长下行笔。",
    ),
    "obs-char-001187": (
        "A faint compact image shows two narrow upright forms above a short horizontal base, with a small upper branch.",
        "较淡的紧凑图像显示短横底笔上方的两个狭长直立形体，并有小型上部分枝。",
    ),
    "obs-char-001188": (
        "Two separated slender strokes are visible, with a branched left form and a longer curved right stroke.",
        "可见两个分离的狭长笔组，左侧形体有分枝，右侧笔较长且弯曲。",
    ),
    "obs-char-001189": (
        "A horizontal rectangular enclosure is crossed by two upper uprights and accompanied by two lower curves.",
        "横向方形框廓被两条上部直立笔穿过，并伴有两条下部弯笔。",
    ),
    "obs-char-001190": (
        "A broad rounded central enclosure has two forked upper branches, short side strokes, and a lower base.",
        "宽大的圆弧形中央框廓有两条分叉上枝、短侧笔和下部底笔。",
    ),
    "obs-char-001191": (
        "The faint compact cluster combines several short angular crossings and a darker pointed lower stroke.",
        "较淡的紧凑笔群结合数个短折角交叉和较深的尖状下部笔。",
    ),
    "obs-char-001192": (
        "Two long curved strokes descend together and meet in a pointed lower fork.",
        "两条长弯笔并行下行，并在下端汇合成尖状分叉。",
    ),
    "obs-char-001193": (
        "A large branching form has a long central descending stroke, a broad left branch, an upper fork, and three detached small marks.",
        "大型分枝形体有中央长下行笔、宽大左侧分枝、上部分叉和三个分离的小型痕迹。",
    ),
    "obs-char-001194": (
        "Two adjacent dense clusters have repeated angular branches and several pointed lower projections.",
        "两个相邻的密集笔群有重复折角分枝和数个尖状下部外伸笔。",
    ),
    "obs-char-001195": (
        "A tall dark curved stroke surrounds a small rounded inner loop and continues into a long tapered lower line.",
        "高而深色的弯曲笔围绕小型圆弧内部环，并继续成为长而渐细的下部线。",
    ),
    "obs-char-001196": (
        "A narrow upright form has a central rectangular enclosure, forked upper and lower branches, and a long descending line.",
        "狭长直立形体有中央方形框廓、上下分叉笔和长下行线。",
    ),
    "obs-char-001197": (
        "Two detached slender branching clusters are visible, with unequal sizes and pointed lower ends.",
        "可见两个分离的狭长分枝笔群，大小不等且下端呈尖状。",
    ),
    "obs-char-001198": (
        "A large dark branching form has a long descending left stroke, a broad upper junction, and two rightward projections.",
        "大型深色分枝形体有长下行左笔、宽大的上部连接处和两条向右外伸笔。",
    ),
    "obs-char-001199": (
        "A faint dense form has two upper branching clusters, a broad horizontal middle, and unequal lower strokes.",
        "较淡的密集形体有两个上部分枝笔群、宽大的中部横笔和不等长下部笔。",
    ),
    "obs-char-001200": (
        "An open rounded form has several upper prongs, a central descending curve, and a hooked lower end.",
        "开放的圆弧形体有数条上部尖枝、中央下行弯笔和钩状下端。",
    ),
    "obs-char-001161": (
