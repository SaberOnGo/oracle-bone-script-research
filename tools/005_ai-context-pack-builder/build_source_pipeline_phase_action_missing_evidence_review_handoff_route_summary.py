#!/usr/bin/env python3
"""Build a route summary for missing-evidence handoff review checklists.

The summary indexes the 153 checklist rows by source and pipeline gap for later
...
DEFAULT_OUTPUT = STAT_DIR / "154_source-pipeline-phase-action-missing-evidence-review-handoff-route-summary.json"
...
CAUTION = (
    "This source pipeline missing-evidence handoff route summary is routing-only. "
...
def route_from_row(index: int, row: dict[str, str]) -> dict[str, object]:
    return {
        "summary_route_id": f"source-pipeline-missing-evidence-review-handoff-summary-route-{index:03d}",
        "handoff_review_checklist_path": SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_HANDOFF_CHECKLIST.as_posix(),
...
def build_route_summary(checklist_rows: list[dict[str, str]]) -> dict[str, object]:
    ...
    return {
        "route_summary_id": "source-pipeline-missing-evidence-review-handoff-route-summary-001",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build source pipeline missing-evidence handoff route summary.")
    parser.add_argument("--checklist", default=str(SOURCE_PIPELINE_PHASE_ACTION_MISSING_EVIDENCE_REVIEW_HANDOFF_CHECKLIST))
    ...
    print(f"missing_evidence_handoff_summary_routes={data['route_count']} output={args.output}")
    return 0
